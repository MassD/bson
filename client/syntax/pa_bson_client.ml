      ...
      List.map (
        fun t ->
          ...
          let functor_params =
            ...
            ) (<:module_expr< $uid:("Bson_ext_client_" ^ name) $>>) params
          in
          ...
          let body =
            <:module_expr<
              struct
              value to_bson t =
                let module $uid:"Bson_ext_client_" ^ name$ = $functor_params$ in
                let elt = $uid:("Bson_ext_client_" ^ name)$.to_bson t in
                Bson.get_doc_element elt;

              value from_bson b =
                let module $uid:"Bson_ext_client_" ^ name$ = $functor_params$ in
                let elt = Bson.create_doc_element b in
                $uid:("Bson_ext_client_" ^ name)$.from_bson elt;
          ...
          let body =
            List.fold_right (
              fun (name,_) body ->
                <:module_expr<
                  functor ($uid:("M" ^ name)$ : $uid:"Bson_ext_client"$.$uid:"Bson_ext_client"$)
                  -> $body$
                >>
            ) params body
          in
